def validate_style(self, value):
    ...
    self._validate_optional_string(value, "title", 200)
    self._validate_optional_string(value, "subtitle", 200)
    self._validate_optional_string(value, "xAxisLabel", 100)
    self._validate_optional_string(value, "yAxisLabel", 100)
    self._validate_colors(value)
    self._validate_legend_position(value)
    self._validate_boolean_fields(value, ["gridlines", "tooltips", "dataLabels"])

    return value


def _validate_optional_string(self, value, field_name, max_length):
...
def _validate_colors(self, value):
    if "colors" not in value or value["colors"] is None:
        return
...
def _validate_legend_position(self, value):
...
def _validate_boolean_fields(self, value, fields):
    for field in fields:
